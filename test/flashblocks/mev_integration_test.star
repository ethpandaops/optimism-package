...
def test_flashblocks_mev_sidecar_conditions(plan):
    ...
    participant_no_mev = struct(
        name="flashblocks-rpc", use_flashblocks=True, mev_params=None
    )
...
def test_flashblocks_mev_external_builder_detection(plan):
    ...
    mev_params_internal = struct(
        enabled=True, type="rollup-boost", builder_host=None, builder_port=None
    )

    is_external_builder = (
        mev_params_internal.builder_host != None
        and mev_params_internal.builder_port != None
    )
    ...
    is_external_builder = (
        mev_params_external.builder_host != None
        and mev_params_external.builder_port != None
    )
    ...
    is_external_builder = (
        mev_params_partial.builder_host != None
        and mev_params_partial.builder_port != None
    )
    expect.eq(is_external_builder, False)
...
def test_flashblocks_mev_context_switching(plan):
    ...
    el_context_for_cl = (
        sidecar_and_builders_none.sidecar.context
        if sidecar_and_builders_none and sidecar_and_builders_none.sidecar
        else el_context
    )
    ...
    sidecar_and_builders_no_sidecar = struct(
        sidecar=None, el_builder=struct(), cl_builder=struct()
    )

    el_context_for_cl = (
        sidecar_and_builders_no_sidecar.sidecar.context
        if sidecar_and_builders_no_sidecar and sidecar_and_builders_no_sidecar.sidecar
        else el_context
    )
...
def test_flashblocks_mev_external_builder_context(plan):
    ...
    mev_params = struct(builder_host="external-builder.example.com", builder_port=8545)
    ...
    expect.eq(
        el_builder_context.rpc_http_url, "http://external-builder.example.com:8545"
    )
...
def test_flashblocks_mev_success_logging(plan):
    ...
    log_message = "Successfully launched with --websocket-url={} {}".format(
        websocket_url, mev_status
    )
...
def test_flashblocks_mev_rollup_boost_type_validation(plan):
    ...
    # Valid MEV type
    mev_params_valid = struct(enabled=True, type="rollup-boost")
    ...
    mev_params_invalid = struct(
        enabled=True, type="flashbots-boost"  # Not rollup-boost
    )
...
def test_flashblocks_mev_component_structure(plan):
    ...
    sidecar_and_builders = struct(
        el_builder=struct(context=struct(service_name="op-rbuilder-instance")),
        cl_builder=struct(context=struct(service_name="op-node-builder-instance")),
        sidecar=struct(
            context=struct(
                service_name="rollup-boost-sidecar",
                rpc_http_url="http://rollup-boost-sidecar:8545",
    ...
    expect.eq(
        sidecar_and_builders.el_builder.context.service_name, "op-rbuilder-instance"
    )
    expect.eq(
        sidecar_and_builders.cl_builder.context.service_name, "op-node-builder-instance"
    )
