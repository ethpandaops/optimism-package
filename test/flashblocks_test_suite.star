...
# Import test modules
flashblocks_input_parser_tests = import_module(
    "/test/flashblocks/input_parser_test.star"
)
...
participant_flashblocks_tests = import_module(
    "/test/l2/participant/flashblocks_test.star"
)
...
def test_flashblocks_complete_suite(plan):
    ...
    flashblocks_input_parser_tests.test_flashblocks_websocket_proxy_parser_disabled(
        plan
    )
    flashblocks_input_parser_tests.test_flashblocks_websocket_proxy_parser_enabled_defaults(
        plan
    )
    flashblocks_input_parser_tests.test_flashblocks_websocket_proxy_parser_custom_image(
        plan
    )
    flashblocks_input_parser_tests.test_flashblocks_websocket_proxy_parser_custom_ports(
        plan
    )
    flashblocks_input_parser_tests.test_flashblocks_websocket_proxy_parser_custom_labels(
        plan
    )
    flashblocks_input_parser_tests.test_flashblocks_websocket_proxy_parser_invalid_args(
        plan
    )
    flashblocks_input_parser_tests.test_flashblocks_websocket_proxy_parser_resource_limits(
        plan
    )
    flashblocks_input_parser_tests.test_flashblocks_websocket_proxy_parser_tolerations_node_selectors(
        plan
    )

    # Launcher Tests
    flashblocks_launcher_tests.test_flashblocks_websocket_proxy_get_service_config_basic(
        plan
    )
    flashblocks_launcher_tests.test_flashblocks_websocket_proxy_get_service_config_with_conductors(
        plan
    )
    flashblocks_launcher_tests.test_flashblocks_websocket_proxy_get_service_config_single_conductor(
        plan
    )
    flashblocks_launcher_tests.test_flashblocks_websocket_proxy_environment_variables(
        plan
    )
    ...
    participant_flashblocks_tests.test_participant_use_flashblocks_disabled_explicitly(
        plan
    )
    participant_flashblocks_tests.test_participant_flashblocks_with_mev(plan)
    participant_flashblocks_tests.test_participant_flashblocks_sequencer_inheritance(
        plan
    )
    participant_flashblocks_tests.test_participant_flashblocks_with_sequencer_reference(
        plan
    )
...
def test_flashblocks_integration_scenarios(plan):
    ...
    production_participants = [
        ...
        struct(
            name="flashblocks-rpc-0", use_flashblocks=True, el=struct(type="op-reth")
        ),
        struct(
            name="flashblocks-rpc-1", use_flashblocks=True, el=struct(type="op-reth")
        ),
        struct(
            name="flashblocks-rpc-2", use_flashblocks=True, el=struct(type="op-reth")
        ),
    ]
...
def _test_participant_separation_scenario(
    plan, participants, expected_regular, expected_flashblocks
):
...
def test_flashblocks_error_conditions(plan):
    ...
    proxy_context = struct(
        context=struct(ws_url="ws://flashblocks-websocket-proxy-2151908-network:8545")
    )
