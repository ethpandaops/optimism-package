...
# "signer" a reserved client name, used to generate credentials for the signer itself
_SIGNER_CLIENT_HOSTNAME = "signer"
    ...
    return ServiceConfig(
        ...
        env_vars={
            ...
            "OP_SIGNER_TLS_CERT": "{}/{}".format(
                ...
                credentials.clients[_SIGNER_CLIENT_HOSTNAME].tls.crt,
            ),
            "OP_SIGNER_TLS_KEY": "{}/{}".format(
                _SIGNER_CREDENTIALS_DIR,
                credentials.clients[_SIGNER_CLIENT_HOSTNAME].tls.key,
            ),
...
def _create_credentials(plan, params, clients, script_artifacts, registry):
    # We need to generate the client credentials for all the services and the signer itself
    client_hostnames = [_SIGNER_CLIENT_HOSTNAME] + [
...
def create_signer_config(plan, params, clients, private_keys, script_artifacts):
...
def _assert_client(client):
    ...
    if client.hostname == _SIGNER_CLIENT_HOSTNAME:
        fail(
            "op-signer: client struct 'hostname' field must not be '{}', got {}".format(
                _SIGNER_CLIENT_HOSTNAME, client.hostname
            )
        )
