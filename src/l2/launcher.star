...
def launch(plan, params, jwt_file, deployment_output, l1_config_env_vars,node_selectors, observability_helper):
    network_params = params.network_params
    network_name = network_params.name

    log_prefix = "L2 network {}".format(network_name)

    plan.print("{}: Launching (network ID {})".format(log_prefix, network_params.network_id))
    ...
    _launch_proxyd_maybe(plan=plan, proxyd_params=params.proxyd_params, participants=params.participants, network_params=network_params, observability_helper=observability_helper)
    _launch_tx_fuzzer_maybe(plan=plan, tx_fuzzer_params=params.tx_fuzzer_params, participants=params.participants, node_selectors=node_selectors)
...
def _launch_proxyd_maybe(plan, proxyd_params, participants, network_params, observability_helper):
    if proxyd_params:
