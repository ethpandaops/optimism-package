    ...
    return _assert_unique_l2_ids(
        _filter.remove_none(
            [
                ...
                for l2_name, l2_args in (args or {
                    # We supply a default network if none was provided
                    "opkurtosis": None
                }).items()
            ]
        )
...
def _parse_instance(l2_args, l2_name, l2_id_generator, registry):
    ...
    l2_params["tx_fuzzer_params"] = _tx_fuzzer_input_parser.parse(
        l2_params["tx_fuzzer_params"], l2_params["network_params"], registry
    )

    # We add the DA params
    l2_params["da_params"] = _da_input_parser.parse(
        l2_params["da_params"], l2_params["network_params"], registry
    )
